...
def load_data(image_features_path, text_data_path):
    '''Loads the COCO dataset. Expects a path to one of its json files and to a
    ...
    print(len(sentences), ' ', len(img_features_feed))
    ...
    return placeholders, feeds, vocabulary


def build_vete_model(placeholders, vocabulary_size):
    model = {'layers': {}, 'placeholders': placeholders}
    ...
    E_bow_gather     = tf.gather(word_embeddings, sentences_ph)
    E_bow_sum        = tf.reduce_sum(E_bow_gather, axis=1)
    E_bow_normalized = tf.nn.l2_normalize(E_bow_sum, axis=1)
    ...
    similarity = cosine_similarity(img_embedding, E_bow_normalized)
    ...
    model['layers']['E_bow_gather']     = E_bow_gather
    model['layers']['E_bow_sum']        = E_bow_sum
    model['layers']['E_bow_normalized'] = E_bow_normalized
...
if __name__ == '__main__':
    placeholders, feeds, vocabulary = load_data(image_features_path,
                                                text_data_path)
